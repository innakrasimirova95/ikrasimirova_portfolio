...
const defaultColors = ['#7E57C2', '#FF9FFC', '#C3B5E5'];
const lightColors = ['#4A4C62', '#4A0D66', '#310A47'];
...
interface ShaderPassProps {
    ...
    type Uniforms = Record<string, { value: THREE.Texture | THREE.Vector2 | THREE.Vector4 | boolean | number }>;
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: SimPropsAdvection) {
          super({
            material: {
              ...
              uniforms: {
                ...
                velocity: { value: (simProps.src && simProps.src.texture) ? simProps.src.texture : new THREE.Texture() },
    ...
    interface SimPropsExternalForce {
      dst: THREE.WebGLRenderTarget | null;
      cellScale: THREE.Vector2;
      cursor_size: number;
    }
    class ExternalForce extends ShaderPass {
      mouse!: THREE.Mesh;
      constructor(simProps: SimPropsExternalForce) {
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: SimPropsViscous) {
        super({
          material: {
            vertexShader: face_vert,
            ...
            uniforms: {
              ...
              velocity: { value: (simProps.src && simProps.src.texture) ? simProps.src.texture : new THREE.Texture() },
              velocity_new: { value: (simProps.dst_ && simProps.dst_.texture) ? simProps.dst_.texture : new THREE.Texture() },
    ...
    class Divergence extends ShaderPass {
      constructor(simProps: SimPropsDivergence) {
        super({
          material: {
            vertexShader: face_vert,
    ...
    class Poisson extends ShaderPass {
      constructor(simProps: SimPropsPoisson) {
        super({
          material: {
            vertexShader: face_vert,
            ...
            uniforms: {
              boundarySpace: { value: simProps.boundarySpace },
              pressure: { value: (simProps.dst_ && simProps.dst_.texture) ? simProps.dst_.texture : new THREE.Texture() },
              divergence: { value: (simProps.src && simProps.src.texture) ? simProps.src.texture : new THREE.Texture() },
              px: { value: simProps.cellScale }
            }
          },
          output: simProps.dst,
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: SimPropsPressure) {
        super({
          material: {
            vertexShader: face_vert,
            ...
            uniforms: {
              boundarySpace: { value: simProps.boundarySpace },
              pressure: { value: (simProps.src_p && simProps.src_p.texture) ? simProps.src_p.texture : new THREE.Texture() },
              velocity: { value: (simProps.src_v && simProps.src_v.texture) ? simProps.src_v.texture : new THREE.Texture() },
              px: { value: simProps.cellScale },
              dt: { value: simProps.dt }
            }
          },
    ...
    class Simulation {
      ...
      update() {
        ...
        this.pressure.update({ vel: vel ?? undefined, pressure: pressure ?? undefined });
      }
    }
